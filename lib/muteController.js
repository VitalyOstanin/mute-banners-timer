...
// Format a remaining-seconds count as m:ss. Pure helper kept here (not in the
// UI module) so it falls under the gjs unit tests.
...
export class MuteController {
  ...
  // `proto` is the MessageTray prototype, passed in rather than read from the
  // shell so the gjs unit tests can install the guard on a fake tray's prototype.
  install(proto) {
  ...
  // the release burst instead of being lost.
  //
  // No public/signal path does this portably: GNOME 46+ re-emits
  // notification-request-banner when `acknowledged` is cleared, but GNOME 45's
  // notify::acknowledged only calls countUpdated. Even on 46+ the re-emit is not
  // enough on its own: while this notification is still the tray's `_notification`,
  // _onNotificationRequestBanner takes the "already showing" branch and never
  // re-queues it. So the shown banner must first be detached from `_notification`
  // (the teardown below), and only then is it pushed back — explicitly on 45,
  // guarded by includes() against the 46+ auto-push.
